				...
				<c:when test="${fn:length(resultList) > 0}">
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr class="cursor">
							...
							<td onclick="fncPageBoard('view','view.do','${result.meSeq}','meSeq')">${result.meCnum }</td>
							<td onclick="fncPageBoard('view','view.do','${result.meSeq}','meSeq')">${result.meBusu }</td>
							<td onclick="fncPageBoard('view','view.do','${result.meSeq}','meSeq')">${result.meLeve }</td>
							<td onclick="fncPageBoard('view','view.do','${result.meSeq}','meSeq')">${result.meMail }</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
